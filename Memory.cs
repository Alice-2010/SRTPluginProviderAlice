...
namespace SRTPluginProviderAlice
{
    internal static class Memory
    {
        ...
        {
            long min = 0x10000L;
            long max = 0x7FFEFFFFL;

            UIntPtr mbiSize = (UIntPtr)Marshal.SizeOf(typeof(MemoryBasicInformation));

            long addr = min;
            do
            {
                if (WinAPI.VirtualQueryEx(process.Handle, (IntPtr)addr, out MemoryBasicInformation mbi, mbiSize) == UIntPtr.Zero)
                {
                    break;
                }
    ...
    public static class WinAPI
    {
        ...
        public static extern UIntPtr VirtualQueryEx(IntPtr hProcess, IntPtr lpAddress,
            [Out] out MemoryBasicInformation lpBuffer, UIntPtr dwLength);
        ...
        [DllImport("kernel32.dll", SetLastError = true)]
        [return: MarshalAs(UnmanagedType.Bool)]
        public static extern bool VirtualProtectEx(IntPtr hProcess, IntPtr lpAddress, UIntPtr dwSize,
        ...
        [DllImport("kernel32.dll", SetLastError = true)]
        public static extern IntPtr CreateRemoteThread(IntPtr hProcess, IntPtr lpThreadAttributes, UIntPtr dwStackSize,
